...
def lambda_handler(event, context):
    ...
    if not any(api_endpoints_list):
        message = f"{function_name}: No API endpoints returned."
    ...
    if not s3_bucket:
        message = f"{function_name}: No S3 bucket returned."
        subject = f"{function_name}: Failed"
    ...
    if endpoint_count_failure > 0:
        message = f"{function_name} ran with {endpoint_count_failure} errors.  Please check logs."
